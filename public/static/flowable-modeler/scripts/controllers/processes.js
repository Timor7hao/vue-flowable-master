    ...
    $scope.ok = function () {
        ...
        $http({method: 'POST', url: FLOWABLE.APP_URL.getModelsUrl(), data: $scope.model.process}).
            success(function(data) {
                ...
                console.log(data.id)
                //添加自己后台，并传值
                console.log(window.parent["mytitle"]())
                console.log(window.parent["myid"]())
                var Params = {
                    form_model_id: window.parent["myid"](),
                    proc_model_id: data.id,
                }
                console.log(Params)
                $http({method: 'PUT', url: '/api/form/model/bind', data: Params}).
                    success(function(data) {
                        console.log(data);
                        alert('表单模型与流程模型绑定成功');
                        // this.$message({
                        //     type: 'success',
                        //     message: '表单模型与流程模型绑定成功' 
                        // });
                    }).
                    error(function(data, status, headers, config) {
                        $scope.model.loading = false;
...
.controller('DuplicateProcessModelCtrl', ['$rootScope', '$scope', '$modal', '$http', '$location',
    ...
    $scope.ok = function () {
        ...
        // $scope.model.loading = false;
        // $rootScope.editorHistory = [];
        // $location.path("/editor/cd5f0f12-9179-11e9-aae2-0a002700000d");
    };
